...
%开仓止损
%%
result={'平仓止损','策略年化收益','基准收益','Beta','Alpha','波动率','夏普比','最大回撤','单次最大收益','单次最大损失','胜率',' 盈亏比','平均收益','平均损失','交易次数','平均每次交易盈利','平均每次交易亏损','平均每次交易资金占用'};rr=2;
for k1=0:0.0005:0.005
    ...
    chart={'开仓日期','平仓日期','A 行权价','IV A','A仓位','B 行权价','IV B','B仓位','总盈亏','平均资金占用','总仓位','持仓天数','收益率','年化收益率','开仓IV差','平仓IV差','平仓剩余天数'};    mm=1;i=1;
    ...
    result{rr,18}=sum(cell2mat(time(2:end,8)))/size(chart,1);
