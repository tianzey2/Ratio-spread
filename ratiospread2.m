...
%%
result={'年份初始','策略年化收益','基准收益','Beta','Alpha','波动率','夏普比','最大回撤','单次最大收益','单次最大损失','胜率',' 盈亏比','平均收益','平均损失','交易次数','平均每次交易盈利','平均每次交易亏损','平均每次交易资金占用'};rr=2;
timelist={'2015/01/01','2015/12/31';'2016/01/01','2016/12/31';'2017/01/01','2017/12/31';'2018/01/01','2018/12/31';'2019/01/01','2019/06/28'};
for tl=1:size(timelist,1)
    time={'date','当日盈亏','总盈亏','收益率','code','组合数量','净值','资金占用','rfree','基准收益','基准收益率','基准净值','净值差比率'};
    list={'A code','B code','详细信息','当日盈亏','资金占用'};
    detail_use={'日期','exe_price A','maint_margin A','ptmtradeday A','iv A','A仓位','A收盘价','exe_price B','maint_margin B','ptmtradeday B','iv B','B仓位','B收盘价','A资金占用','B资金占用','A当日盈亏','B当日盈亏','A delta','B delta','type','当日总盈亏'};
    chart={'开仓日期','平仓日期','A 行权价','IV A','A仓位','B 行权价','IV B','B仓位','总盈亏','平均资金占用','总仓位','持仓天数','收益率','年化收益率','开仓IV差','平仓IV差','平仓剩余天数'};
    mm=1;kk=1;
    for i=2:size(OptionData,1)
        if datenum(OptionData(i,1))<datenum(timelist{tl,1})
         continue
        end
        if  datenum(OptionData(i,1))>datenum(timelist{tl,2})
         break
        end
        kk=kk+1;
        time{kk,1}=OptionData{i,1};  
        time{kk,2}=0;
        time{kk,8}=0;
        k=OptionData{i,4};
        data=[OptionData{i,2}(:,:);OptionData{i,3}(2:end,:)];
        ...
        for a=size(code,1):-1:2
            ...
            if  abs(detail{nn,11}-detail{nn,5})<=y1 || detail{nn,4}<=date1 ||(strcmp(detail{nn,20},OptionData{1,3})&&detail{nn,8}>=k-k1)||(strcmp(detail{nn,20},OptionData{1,2})&&k>=detail{nn,8}-k1)
                    ...
                    detail{nn,21}=detail{nn,16}+detail{nn,17};
                    code{a,4}=detail{nn,16}+detail{nn,17};
                    time{kk,2}=time{kk,2}+code{a,4};
                    mm=mm+1;
                    ...
                    chart{mm,9}=sum(cell2mat(detail(2:end,21)));
        ...
        for M=2:3
          ...
          time{kk,2}=sum(cell2mat(code(2:end,4)));
        if kk~=2
            time{kk,3}=time{kk,2}+time{kk-1,3};
        else
            time{kk,3}=time{kk,2};
        end
        time{kk,8}=sum(cell2mat(code(2:end,5)));
        if time{kk,8}~=0
            time{kk,4}=time{kk,2}/time{kk,8};
        else
            time{kk,4}=0;
        end
        time{kk,6}=size(code,1)-1;
        if kk~=2
            time{kk,7}=(time{kk,4}+1)*time{kk-1,7};
        else
            time{kk,7}=1*(time{kk,4}+1);
        end
        time{kk,9}=OptionData{i,5};
        if kk~=2 
            time{kk,10}=OptionData{i,4}*OptionData{i,6}-OptionData{i-1,4}*OptionData{i-1,6};
            time{kk,11}=(OptionData{i,4}*OptionData{i,6}-OptionData{i-1,4}*OptionData{i-1,6})/(OptionData{i-1,4}*OptionData{i-1,6});
        else
            time{kk,10}=0;
            time{kk,11}=0;
        end
        if kk~=2
            time{kk,12}=(time{kk,11}+1)*time{kk-1,12};
        else
            time{kk,12}=1*(time{kk,11}+1);
        end
        if kk~=2
            time{kk,13}=(time{kk,7}-time{kk-1,7})/time{kk-1,7};
        else
            time{kk,13}=0;
        end     
        time{kk,5}=code;
    ...
    %%%%
    result{rr,1}=timelist{tl,1};
    result{rr,2} = (time{end,7}/time{2,7}-1)*250/size(time,1);
    result{rr,3}=(time{end,12}/time{2,12}-1)*250/size(time,1) ;
    ...
    result{rr,5}=result{rr,2}-rf-result{rr,4}*(result{rr,3}-rf);
    result{rr,6}=std(cell2mat(time(2:end,4)))*sqrt(250);
    result{rr,7}=( result{rr,2}- rf)/result{rr,6};
    result{rr,8}=maxdrawdown(cell2mat(time(2:end,7)));
    result{rr,9}=max(cell2mat(chart(2:end,9)));
    result{rr,10}=min(cell2mat(chart(2:end,9)));
    w=0;
    ...
    result{rr,13}=earning/size(chart(2:end,9),1);
    result{rr,14}=loss/size(chart(2:end,9),1);
    result{rr,12}=abs(earning/loss);
    result{rr,15}=size(chart,1);
    result{rr,16}=earning/size(SS(e+1:end,1),1);
    result{rr,17}=loss/size(SS(1:e,1),1);
    result{rr,18}=sum(cell2mat(chart(2:end,10)))/size(chart,1);
    figure1=plot(cell2mat(time(2:end,7)));
    figure2=plot(cell2mat(time(2:end,3)));
    figure3=plot(cell2mat(time(2:end,4)));
    rr=rr+1;
end
